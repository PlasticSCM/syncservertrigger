...
namespace Codice.CmdRunner
{
    ...
    internal class BaseCmdRunner
    {
            ...
            bool bUseCmShell)
        {
            ...
            try
            {
                int result = (input != null)
                    ? RunAndWaitWithInput(command, path, input, out output, out error)
                    : RunAndWait(command, path, out output, out error, bUseCmShell);
        ...
        internal virtual int RunAndWait(
            string cmd, string workingdir, out string output,
            out string error, bool bUseCmShell)
        {
            return RunAndWaitWithInput(
                cmd, workingdir, null, out output, out error);
        }
        ...
        internal virtual int RunAndWaitWithInput(
            string cmd, string workingdir, string input,
        ...
        private int ManageException(string command, string path, Exception e)
        {
            string errormsg = string.Format(
                "Error executing command {0} on path {1}. Error = {2}",
                command,
                path,
        ...
        {
            WriteLine(string.Format("{0}$ {1}", path, command));
        }
