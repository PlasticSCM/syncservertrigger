...
namespace Codice.CmdRunner
{
    public class CmdRunner
    {
        ...
        {
            string output, error;
            if (runner.InternalExecuteCommand(
                command, path, null, out output, out error, true) != 0)
            {
                throw new Exception(output);
            }
        }

        public static int ExecuteCommandWithInput(
            string command, string path, string input)
        {
            string output, error;
            return runner.InternalExecuteCommand(
                command, path, input, out output, out error, true);
        }
        ...
        public static int ExecuteCommandWithResult(string command, string path)
        {
            string output, error;
            return runner.InternalExecuteCommand(
                command, path, null, out output, out error, true);
        }

        public static int ExecuteCommandWithResult(
            string command, string path, out string output,
            out string error, bool bUseCmShell)
        {
            return runner.InternalExecuteCommand(
                command, path, null, out output, out error, bUseCmShell);
        }
        ...
        public static string ExecuteCommandWithStringResult(
            string command, string path, bool bUseShell)
        {
            string output, error;
            runner.InternalExecuteCommand(
                command, path, null, out output, out error, bUseShell);
            return output;
        }
        ...
        static bool bSetBotMode = false;
        static CodiceCmdRunner runner = new CodiceCmdRunner();
    }
}