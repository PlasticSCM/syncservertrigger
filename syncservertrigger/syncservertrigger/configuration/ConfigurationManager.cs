...
namespace SyncServerTrigger.Configuration
{
    public class ConfigurationManager
    {
        public static int GetInt(
            string configFile, string configKey, int defaultValue)
        {
            int result = GetInt(configFile, configKey);
        ...
        public static int GetInt(string configFile, string configKey)
        {
            string val = GetEntry(configFile, configKey);
        ...
        public static long GetLong(
            string configFile, string configKey, long defaultValue)
        {
            long result = GetLong(configFile, configKey);
        ...
        public static long GetLong(string configFile, string configKey)
        {
            string val = GetEntry(configFile, configKey);
        ...
        public static float GetFloat(
            string configFile, string configKey, float defaultValue)
        {
            float result = GetFloat(configFile, configKey);
        ...
        public static float GetFloat(string configFile, string configKey)
        {
            string val = GetEntry(configFile, configKey);
        ...
        public static double GetDouble(
            string configFile, string configKey, double defaultValue)
        {
            double result = GetDouble(configFile, configKey);

            if (result == double.MinValue) return defaultValue;

            return result;
        }

        public static double GetDouble(string configFile, string configKey)
        {
            string val = GetEntry(configFile, configKey);
        ...
        public static bool GetBool(string configFile, string configKey)
        {
            return GetBool(configFile, configKey, false);
        }

        public static bool GetBool(
            string configFile, string configKey, bool defaultValue)
        {
            string val = GetEntry(configFile, configKey);
        ...
        public static bool HasEntryDefined(string configFile, string configKey)
        {
            string entry = GetEntry(configFile, "", configKey);

            return !string.IsNullOrEmpty(entry);
        }

        public static string GetEntry(string configFile, string configKey)
        {
            return GetEntry(configFile, "", configKey);
        }

        public static string GetEntry(
            string configFile, string configSection, string configKey)
        {
            ConfigurationFile file = GetConfigFile(configFile);

            return (file == null) ? null : file.GetEntryValue(configSection, configKey);
        }

        public static void SetEntry(
            string configFile, string configKey, string val)
        {
            SetEntry(configFile, "", configKey, val);
        }

        public static void SetEntry(
            string configFile, string configSection, string configKey, string val)
        {
            ConfigurationFile file = GetConfigFile(configFile);

            if (file != null)
            {
                file.SetEntryValue(configSection, configKey, val);
            }
        }

        public static ConfigurationSection GetSection(
            string configFile, string configSection)
        {
            ConfigurationFile file = GetConfigFile(configFile);

            return (file == null) ? null : file.GetSection(configSection);
        }

        public static void SaveAll()
        {
            foreach (ConfigurationFile file in mConfigFiles.Values)
            {
                file.Save();
            }
        }
        ...
        public static void ReloadFile(string configFile)
        {
            mConfigFiles[configFile] = LoadConfigFile(configFile);
        }

        public static void SetEmptyConfigFile(string configFile)
        {
            mConfigFiles[configFile] = new ConfigurationFile(configFile);
        }

        public static void ResetConfigFileForTesting(string configFile)
        {
            mConfigFiles.Remove(configFile);
        }

        static ConfigurationFile GetConfigFile(string configFile)
        {
            ConfigurationFile result;

            if (mConfigFiles.TryGetValue(configFile, out result))
                return result;

            result = LoadConfigFile(configFile);
            mConfigFiles.Add(configFile, result);
        ...
        static ConfigurationFile LoadConfigFile(string configFile)
        {
            ...
            ConfigurationFile result = TryLoadOn(appPath, configFile);

            if (result != null)
                return result;

            string defaultPath =
                Utils.GetHomeBasedDirectoryPath(ConfigDirectory);

            result = TryLoadOn(defaultPath, configFile);
        ...
        static Dictionary<string, ConfigurationFile> mConfigFiles
            = new Dictionary<string, ConfigurationFile>();

        const string ConfigDirectory = "syncservertrigger";
    }
